...
export default function AdminSidebar({
  ...
  const avatarInitial = (user?.user_metadata?.display_name ||
    user?.user_metadata?.name ||
    user?.user_metadata?.full_name ||
    userEmail ||
    "A")[0].toUpperCase();
      ...
      <nav aria-label="Admin navigation" className="flex-1 space-y-1.5 px-2">
        {navItems.map(item => (
          <button
            ...
            <div
              className={`w-5 h-5 flex items-center justify-center flex-shrink-0 ${isItemActive(item) ? "text-white" : "text-white/90"}`}>
              <IconSVG type={item.iconType} className="w-full h-full" />
            </div>
            <span
              className={`font-medium ${item.path === "/admin" && isItemActive(item) ? "tracking-[0.03em]" : ""}`}>
              {item.label}
            </span>
          </button>
        ))}
      ...
      <div className="px-3 pt-4 pb-5 border-t border-white/15">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-md hover:bg-white/8 transition-colors mb-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[8px] text-white/60 tracking-[0.05em] uppercase mt-0.5 font-medium">
              Administrador
            </p>
          </div>
        </div>
